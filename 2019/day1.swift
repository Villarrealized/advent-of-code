...
let moduleMasses = [132897, 131436, 107839, 98498, 141198, 147530, 65491, 142162, 89575, 95090, 147097, 129782,
...
let fuelRequirement = moduleMasses
  .map(calculateFuel(for:))
  .reduce(0, +)

print("Part 1: \(fuelRequirement)")


func calculateTotalFuel(for mass: Int) -> Int {
  let fuel = calculateFuel(for: mass)
  return fuel > 0 ? fuel + calculateTotalFuel(for: fuel) : 0
}

let completeFuelRequirement = moduleMasses
  .map(calculateTotalFuel(for:))
  .reduce(0, +)

print("Part 2: \(completeFuelRequirement)")
