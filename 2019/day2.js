...
by looking at the first integer (called position 0). Here, you will find an opcode - either 1, 2, or 99.
...
// Each opcode is represented by a 4 integer sequence
// opcode [1] = add the values of ints at the positions of the next two ints in the opcode
// opcode [2] = same as opcode 1, but multiply
// opcode [99] = program is finished

function processIntcode(intcode, input1, input2) {
  if (!Array.isArray(intcode)) return null;
  ...
  for(let i = 0; i < code.length; i += opcodeLength) {
    const firstValue = code[code[i + 1]];
    const secondValue = code[code[i + 2]];
    const newPosition = code[i + 3];
    if (code[i] === 1) {
      code[newPosition] = firstValue + secondValue;
    } else if (code[i] === 2) {
      code[newPosition] = firstValue * secondValue;
    } else if (code[i] === 99) {
      return code;
    } else {
      console.error(`Invalid opcode: ${code[i]}`)
      return null;
    }
  }
}

// The answer is at position 0 of the array
const restoredProgram = processIntcode(gravityAssistIntcode, 12, 2);
console.log('Part 1:', restoredProgram && restoredProgram[0]);
...
make sure you first reset the computer's memory to the values in the program (your puzzle input) - in other words, don't reuse
...
function findNounAndVerb(forOutput, intcode, noun = 0, verb = 0) {
  const result = processIntcode(intcode, noun, verb)
  if (result && result[0] === forOutput) {
    return { noun, verb };
  }
  // iterate through all possibilities 0-99
  // for noun and verb -- 100 * 100 = 10,000 possible combinations
  verb = verb + 1;
  if (verb === 99) {
    verb = 0;
    noun = noun + 1;
  ...
  if (noun === 99) {
    console.log(`Could not find noun & verb for output: ${forOutput}`);
    return null;
  }
  return findNounAndVerb(forOutput, intcode, noun, verb);
}

const output = 19690720;
const result = findNounAndVerb(output, gravityAssistIntcode);
const answer = result && (100 * result.noun) + result.verb;
